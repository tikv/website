...
Note that the "position" here is different from "machine". In general, PD can only ensure that replicas of a Region are not at a same machine, thereby avoiding that the peer's failure causes more than one replicas to become lost. However in production, you might have the following requirements:
