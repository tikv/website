...
If there is a big difference in the rating of different stores, you need to examine the operator-related metrics, with a special focus on the generation and execution of operators. There are two main situations:
...
- A lot of tables have been created and then emptied (including truncated tables). These empty regions cannot be merged if the `split-table` option is enabled. You can disable this attribute by adjusting the following parameters:
